...
module.exports = function(grunt) {
  ...
  function Runner() {
    ...
    this.nameArgs = [];
    // A list of changed repos to feed to task runs for livereload
    this.changedrepos = Object.create(null);
  }
  ...
  Runner.prototype.init = function init(name, defaults, done) {
    ...
    self.name = name || grunt.task.current.name || 'watch-svn';
  ...
  Runner.prototype._getTargets = function _getTargets(name) {
    ...
    var targets = (onlyTarget ? [onlyTarget] : Object.keys(config)).filter(function(key) {
      ...
      grunt.task.current.requiresConfig([name, target, 'repos']);
    ...
    if (typeof config.repos === 'string' || Array.isArray(config.repos)) {
      var cfg = {
        repos: config.repos,
        tasks: config.tasks,
        name: 'default',
  ...
  Runner.prototype.complete = function complete() {
    ...
    self.changedrepos = Object.create(null);
